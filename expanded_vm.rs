pub mod vm {
    pub mod interpreter {
                    ...
                    }
                    {
                        ...
                        for (_, el) in ordered_inputs.iter().enumerate() {
                            unsafe {
                                current_offset
                                    += write_input_value(
                                        frame.data.offset(current_offset),
                                        el,
                                        &mut vm,
                                    )
                            };
        ...
        fn write_input_value(
            ptr: *mut u64,
            el: &InputValueOrdered,
            vm: &mut VM,
        ) -> isize {
            match el {
                InputValueOrdered::Field(field_element) => {
                    unsafe {
                        *(ptr as *mut Field) = *field_element;
                    }
                    return 4;
                }
                    ...
                    if vec.len() == 0 {
                        ...
                        match &vec[0] {
                            ...
                            _ => {
                                ::core::panicking::panic_fmt(
                                    format_args!(
                                        "Only field elements are supported in arrays for now",
                                    ),
                                );
                            }
                        }
    ...
    pub mod bytecode {
        ...
        pub fn tuple_proj_handler(pc: *const u64, frame: Frame, vm: &mut VM) {
            let mut current_field_offset = 1isize;
            let res = unsafe {
                frame.read_u64_mut(*pc.offset(current_field_offset) as isize)
            };
            ...
            tuple_proj(res, tuple, index, child_sizes, vm, dummy);
            let pc = unsafe { pc.offset(current_field_offset) };
            dispatch(pc, frame, vm)
        }
        ...
        impl OpCode {
            pub fn to_binary(
                ...
                match self {
                    ...
                    Self::TupleProj { res, tuple, index, child_sizes, dummy } => {
                        binary.push(28u64);
                        let mut init_offset = -1isize;
                        binary.push(res.0 as u64);
                        init_offset -= 1;
                        binary.push(tuple.0 as u64);
                        init_offset -= 1;
                        binary.push(*index);
                        init_offset -= 1;
                        binary.push(child_sizes.len() as u64);
                        init_offset -= 1;
                        for elem in child_sizes {
                            binary.push(*elem as u64);
                            init_offset -= 1;
                        }
                        binary.push(*dummy);
                        init_offset -= 1;
        ...
        impl std::fmt::Display for OpCode {
            fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                match self {
                    ...
                    OpCode::TupleProj { res, tuple, index, child_sizes, dummy } => {
                        let mut fields = String::new();
                        fields = ::alloc::__export::must_use({
                            ::alloc::fmt::format(format_args!("{0} %{1}", fields, res.0))
                        });
                        fields = ::alloc::__export::must_use({
                            ::alloc::fmt::format(
                                format_args!("{0} %{1}", fields, tuple.0),
                            )
                        });
                        fields = ::alloc::__export::must_use({
                            ::alloc::fmt::format(format_args!("{0} {1}", fields, index))
                        });
                        fields = ::alloc::__export::must_use({
                            ::alloc::fmt::format(format_args!("{0} _", fields))
                        });
                        fields = ::alloc::__export::must_use({
                            ::alloc::fmt::format(format_args!("{0} {1}", fields, dummy))
                        });
                        f.write_fmt(format_args!("{0}{1}", "tuple_proj", fields))
                            .unwrap();
                    }
        ...
        pub const DISPATCH: [Handler; 47usize] = [
            jmp_handler,
            jmp_if_handler,
            call_handler,
            ret_handler,
